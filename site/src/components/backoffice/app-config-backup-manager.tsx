...
export const AppConfigBackupManager = () => {
  ...
  const apply = async () => {
    try {
      ...
      setMessage(r.error ? r.error.message : "Restore appliqué.");
    } catch {
  ...
  return (
    <Card className="space-y-3 p-4">
      <h2 className="font-semibold">Backup / Restore app_config</h2>
      <textarea value={json} onChange={(e) => setJson(e.target.value)} className="h-52 w-full rounded border p-2 font-mono text-xs" />
      <div className="flex gap-2">
        <Button onClick={load}>Charger backup</Button>
        <Button variant="outline" onClick={apply}>Appliquer restore</Button>
      </div>
