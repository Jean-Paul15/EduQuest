...
export const WeeklyRewardsManager = () => {
  ...
  const save = async () => {
    ...
    setMessage(r.error ? r.error.message : "Politique hebdo enregistrée.");
  };
  ...
  return (
    <Card className="space-y-3 p-4">
      <h2 className="font-semibold">Gagnant hebdomadaire & cadeau</h2>
      <input value={period} onChange={(e) => setPeriod(e.target.value)} className="w-full rounded border p-2 text-sm" />
      <div className="grid gap-2 md:grid-cols-2">
        <select value={policy.reward_kind} onChange={(e) => setPolicy({ ...policy, reward_kind: e.target.value })} className="rounded border p-2 text-sm"><option value="XP">XP</option><option value="TICKET_HALF_7D">TICKET_HALF_7D</option><option value="TICKET_FULL_7D">TICKET_FULL_7D</option></select>
        <input type="number" min={0} value={policy.xp_amount} onChange={(e) => setPolicy({ ...policy, xp_amount: Number(e.target.value) || 0 })} className="rounded border p-2 text-sm" />
        <input type="number" min={1} value={policy.min_score} onChange={(e) => setPolicy({ ...policy, min_score: Number(e.target.value) || 1 })} className="rounded border p-2 text-sm" />
        <input value={policy.reward_note} onChange={(e) => setPolicy({ ...policy, reward_note: e.target.value })} className="rounded border p-2 text-sm" />
      </div>
      <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={policy.enabled} onChange={(e) => setPolicy({ ...policy, enabled: e.target.checked })} /> Cadeau actif</label>
      ...
      {rows.map((x) => <div key={x.profile_id} className="flex items-center justify-between rounded border p-2 text-sm"><p>#{x.rank} • {x.profile_id} • {x.score} pts</p><Button variant="outline" onClick={() => gift(x.profile_id)} disabled={x.reward_granted}>{x.reward_granted ? "Attribué" : "Donner cadeau"}</Button></div>)}
