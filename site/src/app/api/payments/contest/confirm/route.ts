...
export async function POST(request: Request) {
  ...
  const body = (await request.json()) as { contestId?: string; profileId?: string; idempotencyKey?: string };
  if (!body.contestId || !body.profileId) {
    return NextResponse.json({ message: "Payload invalide." }, { status: 400 });
  }
  if (!isUuid(body.contestId) || !isUuid(body.profileId)) {
    return NextResponse.json({ message: "IDs invalides." }, { status: 400 });
  }
  const key = body.idempotencyKey ?? request.headers.get("x-idempotency-key") ?? `contest:${body.contestId}:profile:${body.profileId}`;
  const claim = await claimIdempotency(
    "api:webhook:contest:confirm",
    key,
    body.profileId,
    `${body.contestId}:${body.profileId}`,
  );
  ...
  try {
    ...
    const { data, error } = await admin.rpc("confirm_contest_payment", {
